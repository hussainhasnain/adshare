...
document.addEventListener("DOMContentLoaded", function () {
    // Get the profile div with the specified ID
    let profileDiv = document.getElementById('users__list--profile-sort');

    // Add click event listener to the specific profile div
    profileDiv.addEventListener('click', function () {
        // Find the parent container for the rows
        let userListContainer = profileDiv.closest('.users__list--content');

        // Find all user rows inside the container (excluding the header row)
        let userRows = Array.from(userListContainer.querySelectorAll('.users__list:not(:first-child)'));

        // Get the index of the clicked profile div
        let columnIndex = Array.from(profileDiv.parentElement.children).indexOf(profileDiv);

        // Determine the sorting order (ascending or descending)
        let sortOrder = profileDiv.dataset.sortOrder || 'asc';

        // Sort the rows based on the profile text
        userRows.sort(function (a, b) {
            let aValue = a.children[columnIndex].textContent.trim().toLowerCase();
            let bValue = b.children[columnIndex].textContent.trim().toLowerCase();
        ...
        profileDiv.dataset.sortOrder = sortOrder === 'asc' ? 'desc' : 'asc';
...
//   var popup = document.querySelector('.profile__formBox');
  
//   // Calculate the position to center the popup horizontally
//   var leftPos = (window.innerWidth - popup.offsetWidth) / 2;
  
//   // Calculate the position to center the popup vertically
//   var topPos = (window.innerHeight - popup.offsetHeight) / 2;
  
//   // Set the CSS properties to center the popup
//   popup.style.left = leftPos + 'px';
//   popup.style.top = topPos + 'px';
// }

// // Call the centerPopup function when the window is resized
// window.addEventListener('resize', centerPopup);

// // Call the centerPopup function initially to center the popup
// centerPopup();
...
//   // Set the CSS properties to center the popup
//   popup.style.left = leftPos + 'px';
//   popup.style.top = topPos + window.pageYOffset + 'px';

//   // Adjust overlay height
//   overlay.style.height = document.body.clientHeight + 'px';
// }

// // Function to handle window resize and scroll events
...
// var editButtons = document.querySelectorAll('.users__list--action-edited');
...
// Variables for closing the popup
// const profileFormCrossEl = document.querySelector(".profile__formBox--cross");
// const overlayEl = document.querySelector(".overlay");
...
function showPopup() {
    const overlay = document.querySelector('.overlay');
    const popup = document.querySelector('.profile__formBox');
...
function showPopupDelete() {
  const overlay = document.querySelector('.overlay');
  const popup = document.querySelector('.profile__formBox--delete');
...
function showPopupAdd() {
  const overlay = document.querySelector('.overlay');
  const popup = document.querySelector('.profile__formBox--addUsers');
...
// Event listener to close the popup when the "Esc" key is pressed
...
deleteUsersEl.forEach(function(button) {
  button.addEventListener('click', function() {
    showPopupDelete();
  });
});
